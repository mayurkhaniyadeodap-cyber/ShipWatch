...
import KpiCards, { type Kpi } from "@/frontend/components/kpi-cards";
import FilterBar from "@/frontend/components/filter-bar";
import CourierChips from "@/frontend/components/courier-chips";
import DataTable, { type SortState } from "@/frontend/components/data-table";
import Drawer from "@/frontend/components/drawer";
import { useLiveFill } from "@/frontend/use-live-fill";
import {
  EMPTY_FILTERS,
  filterParams,
  fmtInt,
...
export default function Dashboard() {
  ...
  // On-demand: progressively fill live courier status for EVERY row matching the
  // current tab + filters (not just the sweep's bounded batch). Runs once the list
  // has loaded; keyed on tab+filters, so paging/sorting doesn't restart it.
  const fill = useLiveFill(tab, filters, !!list.data && !list.isError);

  // Overlay the fill's patches onto the current page's rows so cells pop in as the
  // courier answers, without refetching the list on every chunk.
  const rows = useMemo(() => {
    const base = list.data?.rows ?? [];
    if (fill.patches.size === 0) return base;
    return base.map((r) => {
      const p = fill.patches.get(r.id);
      return p ? { ...r, ...p } : r;
    });
  }, [list.data?.rows, fill.patches]);
  const filledIds = useMemo(() => new Set(fill.patches.keys()), [fill.patches]);
  ...
  return (
    <div className="min-h-screen min-w-[1280px]">
      ...
      <main className="flex flex-col gap-[14px] px-7 pb-7 pt-5">
        ...
        {fill.active && (fill.remaining ?? 0) > 0 && (
          <div className="flex items-center gap-[10px] rounded-[10px] border border-brand-border bg-brand-tint px-4 py-[10px] text-[12.5px] font-medium text-brand">
            <span
              aria-hidden
              className="h-[13px] w-[13px] shrink-0 animate-spin rounded-full border-[2px] border-brand/30 border-t-brand"
            />
            <span>
              Fetching live courier status for this view…{" "}
              <span className="tabular-nums font-semibold">{fmtInt(fill.remaining ?? 0)}</span> left
            </span>
          </div>
        )}

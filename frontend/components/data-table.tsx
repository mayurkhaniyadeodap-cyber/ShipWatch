...
// Table-level meta for the live-fill progress, read by the Courier-status cell so
// blank rows still awaiting the courier render "Fetching…" instead of a bare dash.
...
/** Attempts as "panel / courier": the panel count beside the courier-verified
 *  count from live scans. The panel value overflows at 127, so when the two
 *  disagree the panel number is flagged red and the courier number (trustworthy)
 *  is emphasised. Shows the panel value alone until a courier count exists. */
function attemptsCell(panel: number | null, courier: number | null): React.ReactNode {
  const p = panel ?? 0;
  if (courier == null) {
    return (
      <span title="Shipping-panel attempts — no courier scan count yet" className="tabular-nums text-ink">
        {p}
      </span>
    );
  }
  const disagree = attemptsDisagree(panel, courier);
  return (
    <span
      title={
        disagree
          ? `Panel says ${p}, but the courier's scans show ${courier} failed ${
              courier === 1 ? "attempt" : "attempts"
            }`
          : `Panel ${p} · courier ${courier}`
      }
      className="inline-flex items-center justify-end gap-[3px] tabular-nums"
    >
      <span className={disagree ? "font-semibold text-sev-red-fg" : "text-ink"}>{p}</span>
      <span className="text-muted">/</span>
      <span className="font-semibold text-brand">{courier}</span>
    </span>
  );
}
...
/** Spinner + "Fetching…", for a courier cell whose live status is still in flight. */
function fetchingCell(): React.ReactNode {
  return (
    <span className="inline-flex items-center gap-[5px] text-[11px] text-sub">
      <span
        aria-hidden
        className="h-[9px] w-[9px] shrink-0 animate-spin rounded-full border-[1.5px] border-line-2 border-t-sub"
      />
      Fetching…
    </span>
  );
...
export default function DataTable({
  ...
  hasFilters: boolean;
  onClearFilters: () => void;
  /** The on-demand live-fill loop is running for this view. */
  filling?: boolean;
  /** Ids the fill has already resolved (so blank-but-done rows show "—", not a spinner). */
  filledIds?: Set<number>;
}) {
  ...
  const columns = useMemo<ColumnDef<OrderApiRow, unknown>[]>(() => {
    ...
    const attempts = col.accessor("attempt_count", {
      header: () => (
        <span title="Shipping panel / courier-verified failed attempts">Attempts</span>
      ),
      meta: { sortKey: "attempts", align: "right" } as Meta,
      cell: (info) => attemptsCell(info.getValue(), info.row.original.courier_attempts),
    });
    ...
    const courierStatus = col.display({
      ...
      cell: (info) => {
        const m = info.table.options.meta;
        const live = info.row.original.courier_live_status;
        // Blank and still queued in the fill loop → show progress, not a dash.
        if (!live && m?.filling && !m.filledIds?.has(info.row.original.id)) {
          return fetchingCell();
        }
        return courierStatusCell(live, info.row.original.status);
      },
    });

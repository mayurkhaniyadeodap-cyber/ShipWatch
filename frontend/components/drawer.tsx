...
/** "127" alone, or "127 / 1" when a courier-verified count is available. */
function attemptsText(panel: number, courier: number | null): string {
  return courier == null ? String(panel) : `${panel} / ${courier}`;
}
...
export default function Drawer({ tab, row, onClose }: { tab: Tab; row: OrderApiRow; onClose: () => void }) {
  ...
  const fields: Array<[string, string]> = [
    ...
    ["Attempts", attemptsText(row.attempt_count, row.courier_attempts)],
        ...
        <div className="flex flex-col gap-[14px] overflow-auto px-5 py-4">
          ...
          {breached && (
            <div className="rounded-lg bg-sev-red-bg px-3 py-[10px] text-[12.5px] font-semibold text-sev-red-fg">
              ...
              {(() => {
                // Prefer the courier-verified count — the panel's overflows at 127.
                const n = row.courier_attempts ?? row.attempt_count;
                return isNdr && n > 0 ? ` · ${n} failed ${n === 1 ? "attempt" : "attempts"}` : "";
              })()}
            </div>
          )}
...
function StatusComparison({
  ...
  // Failed-attempt counts: courier column is live from the courier's scans; panel
  // column is the (unreliable, 127-capped) shipping-panel figure.
  const courierAttempts = live && typeof live.attempts === "number" ? live.attempts : null;
  const attemptsClash = attemptsDisagree(row.attempt_count, courierAttempts);

  const rows: Array<[string, string | null, string | null]> = [
    ["Status", courierStatus, panelStatus],
    [
      "Attempts",
      courierAttempts != null ? String(courierAttempts) : null,
  ...
  return (
    <div>
      <div className="mb-2 flex items-center gap-2">
        ...
        {(mismatch || attemptsClash) && (
          <span
            title={
              attemptsClash && !mismatch
                ? "Panel and courier disagree on the number of attempts"
                : "Panel and courier disagree"
            }
            className="rounded-full bg-sev-red-bg px-[7px] py-[1px] text-[10px] font-semibold text-sev-red-fg"
          >
            Mismatch
          </span>
        )}
        ...
        <table className="w-full border-collapse text-[12px]">
          ...
          <tbody>
            {rows.map(([label, courierVal, panelVal], i) => {
              const highlight =
                (label === "Status" && mismatch) || (label === "Attempts" && attemptsClash);
              return (
                <tr key={label} className={i % 2 ? "bg-zebra" : "bg-white"}>
                  ...
                  >
                    {loading && (label === "Status" || label === "Attempts") ? (

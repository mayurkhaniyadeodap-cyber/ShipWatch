...
/** How many sweep rows may fall back to the panel (get_order_status) per sync
 *  when the direct courier API answers but can't see the docket. This is the
 *  bulk counterpart of the drawer's panel fallback, and it exists for the same
 *  reason: DTDC scopes tracking to the booking account, so AWBs from an account
 *  with no tracking token (~300 in-flight GL13259 orders as of 2026-07)
 *  otherwise keep a NULL courier status forever. Bounded because each fallback
 *  is one extra MCP call against the portal; at the default the backlog clears
 *  in a few syncs and the steady state is a trickle. 0 disables. */
const panelFallbackRaw = Number(process.env.LIVE_TRACK_PANEL_FALLBACK_MAX ?? NaN);
export const LIVE_TRACK_PANEL_FALLBACK_MAX =
  Number.isFinite(panelFallbackRaw) && panelFallbackRaw >= 0 ? panelFallbackRaw : 100;
...
// ---- On-demand "fill this view" (interactive) --------------------------------
// The background sweep prioritises TAT breaches + NDR and is bounded per sync, so
// normal in-transit rows (and the NDR tail) can sit blank for a long rotation.
// The dashboard offers an on-demand fill: when a tab/filter is viewed, the client
// walks the FULL matching set in chunks (GET /api/live-fill) and live-tracks every
// blank/stale row until the column is populated. It targets blanks (never-checked)
// and rows older than LIVE_FILL_MAX_AGE_MS only, so it CONVERGES and stops rather
// than competing with the sweep as a second continuous refresher.

/** Rows a single /api/live-fill request fills. Small so each request returns in a
 *  few seconds and the column fills visibly; the client loops until none remain. */
export const LIVE_FILL_CHUNK = Number(process.env.LIVE_FILL_CHUNK ?? 0) || 25;
/** Hard ceiling on the chunk one request may fill, regardless of the `limit`
 *  query param, so the endpoint can't be turned into an unbounded outbound burst. */
export const LIVE_FILL_MAX_CHUNK = Number(process.env.LIVE_FILL_MAX_CHUNK ?? 0) || 50;
/** A row checked more recently than this is left alone by the fill (the sweep
 *  keeps it fresh). Only NULL or older-than-this rows are (re)fetched — this is
 *  what lets the fill converge to "remaining: 0" instead of treadmilling forever
 *  on rows it just refreshed. */
export const LIVE_FILL_MAX_AGE_MS =
  Number(process.env.LIVE_FILL_MAX_AGE_MS) || 6 * 60 * 60 * 1000; // 6h
/** Max concurrent courier calls inside one fill chunk. Paced by the same global +
 *  per-courier rate gates the sweep uses (LIVE_TRACK_SPACING_MS / _COURIER_). */
export const LIVE_FILL_CONCURRENCY = Number(process.env.LIVE_FILL_CONCURRENCY) || 6;
/** Ceiling on concurrent /api/live-fill executions across ALL users/tabs. The
 *  outbound rate is already bounded by a PROCESS-GLOBAL courier gate (so N loopers
 *  share one sweep's worth of budget, not N×), but this caps how many requests may
 *  queue behind that gate at once: beyond it the endpoint returns `busy` instantly
 *  and the client backs off, instead of parking thousands of workers on timers. */
export const LIVE_FILL_MAX_INFLIGHT = Number(process.env.LIVE_FILL_MAX_INFLIGHT) || 4;

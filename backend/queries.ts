...
import { getDb, type OrderRow } from "@/backend/db";
import {
  DAYS_PAST_EDD_SQL,
...
import { SEVERITY_BUCKETS, PAGE_SIZES } from "@/backend/config";
...
const SORTABLE: Record<string, string> = {
  ...
  courier_attempts: "courier_attempts",
...
export type FillCandidate = { id: number; awb: string; shipping_company: string | null };

/** Next chunk (+ total remaining count) of rows in this tab/filter whose courier
 *  status is blank or stale — i.e. `courier_live_checked_at` is NULL or older than
 *  `staleBeforeIso` — restricted to the couriers the caller says are fillable
 *  (has a direct integration and isn't benched). Ordered blanks-first, then
 *  least-recently-checked, so the on-demand fill drains the column front-to-back
 *  and always converges. `limit <= 0` returns just the remaining count (no rows). */
export function fillCandidates(
  tab: Tab,
  f: Filters,
  fillableCouriers: string[],
  staleBeforeIso: string,
  limit: number
): { rows: FillCandidate[]; remaining: number } {
  const db = getDb();
  if (fillableCouriers.length === 0) return { rows: [], remaining: 0 };
  const { where, params } = buildWhere(tab, f);
  const keys = fillableCouriers.map((_, i) => `:fc${i}`);
  const p: Record<string, unknown> = { ...params, staleBefore: staleBeforeIso };
  fillableCouriers.forEach((c, i) => (p[`fc${i}`] = c));
  const staleWhere = `${where}
     AND awb IS NOT NULL AND awb <> ''
     AND shipping_company IN (${keys.join(",")})
     AND (courier_live_checked_at IS NULL OR courier_live_checked_at < :staleBefore)`;

  const countSql = `SELECT COUNT(*) AS n FROM orders WHERE ${staleWhere}`;
  const remaining = (db.prepare(countSql).get(bind(countSql, p)) as { n: number }).n;
  if (limit <= 0) return { rows: [], remaining };

  const rowsSql = `SELECT id, awb, shipping_company FROM orders WHERE ${staleWhere}
     ORDER BY (courier_live_checked_at IS NULL) DESC, courier_live_checked_at ASC, id ASC
     LIMIT :fillLimit`;
  const rows = db.prepare(rowsSql).all(bind(rowsSql, { ...p, fillLimit: limit })) as FillCandidate[];
  return { rows, remaining };
}

/** Thrown when an export would materialize more rows than is safe for the (synchronous)
 *  event loop. The route turns this into a 413 asking the user to narrow filters. */
export class ExportTooLarge extends Error {
  constructor(public readonly count: number, public readonly max: number) {
    super(`Export too large: ${count} rows exceeds the ${max} cap`);
    this.name = "ExportTooLarge";
  }
}

// better-sqlite3 is synchronous and exceljs builds cells in a tight loop, so a huge
// result set blocks the single Node event loop (freezing the UI + the sync loop).
// The TAT/NDR tabs realistically match ~thousands of rows; this cap only trips on a
// pathological/abusive request.
const EXPORT_MAX_ROWS = 100_000;

/** ALL matching rows for Excel export (no pagination, but bounded to protect the event loop). */
export function exportRows(tab: Tab, f: Filters): ListRow[] {
  const db = getDb();
  const { where, params } = buildWhere(tab, f);
  const countSql = `SELECT COUNT(*) AS n FROM orders WHERE ${where}`;
  const n = (db.prepare(countSql).get(bind(countSql, params)) as { n: number }).n;
  if (n > EXPORT_MAX_ROWS) throw new ExportTooLarge(n, EXPORT_MAX_ROWS);
...
// ---- Aggregate cache -------------------------------------------------------
//
// meta() / tatKpis() / ndrKpis() are portal-wide rollups (DISTINCT lists, GROUP
// BYs, full-table COUNTs) that only change when a sync writes new rows. Recomputing
// them on every dashboard load needlessly re-scans the cache AND — because
// better-sqlite3 is synchronous — contends with the sync's own writes, which is a
// big part of why the dashboard stalled for seconds during a sync. Cache them by
// the last SUCCESSFUL sync timestamp: a running sync hasn't logged success yet, so
// the key is stable while it runs (aggregates stay instant), and flips exactly once
...
function cachedAgg<T>(name: string, compute: () => T): T {
  ...
  const value = compute();
  aggCache.set(name, { key, value });
  return value;
}

export function tatKpis() {
  return cachedAgg("kpis:tat", computeTatKpis);
}
export function ndrKpis() {
  return cachedAgg("kpis:ndr", computeNdrKpis);
}
export function meta() {
  return cachedAgg("meta", computeMeta);
}

function computeTatKpis() {
  const db = getDb();

...
const ORDER_COLUMNS = `
  ...
  courier_live_status TEXT, courier_live_checked_at TEXT, courier_attempts INTEGER
`;
...
const ADDED_COLUMNS: Array<{ name: string; type: string }> = [
  ...
  // Failed delivery attempts counted from the courier's own scans (the panel's
  // attempt_count overflows at 127). App-owned like the courier_live_* pair.
  { name: "courier_attempts", type: "INTEGER" },
];
...
function open(): Database.Database {
  ...
  db.exec(`
    ...
    -- PARTIAL index for the TAT-breach predicate (TAT_BREACH_SQL): edd over just the
    -- ~44k undelivered rows. The TAT list, KPIs, meta, courier chips and the live-fill
    -- all filter on "edd < :today AND <undelivered>"; without this the planner scans
    -- ~950k rows via idx_orders_edd every call (~3.6s). With it, ~0.9ms. The WHERE
    -- MUST match UNDELIVERED_SQL exactly or the planner won't use it for that predicate.
    CREATE INDEX IF NOT EXISTS idx_orders_tat
      ON orders(edd)
      WHERE status NOT IN ('Delivered','Cancelled') AND status NOT LIKE 'RTO%';
    -- customer_state has no natural index; meta's DISTINCT state list scanned ~950k
    -- rows (~1.7s) on every dashboard load. Index-backed DISTINCT drops it to ~0.1s.
    CREATE INDEX IF NOT EXISTS idx_orders_cstate ON orders(customer_state);
...
export const ORDER_FIELD_NAMES = [
  ...
  "courier_live_status", "courier_live_checked_at", "courier_attempts",
] as const;
...
export const PORTAL_OWNED_COLUMNS = ORDER_FIELD_NAMES.filter(
  (c) =>
    c !== "id" &&
    c !== "courier_live_status" &&
    c !== "courier_live_checked_at" &&
    c !== "courier_attempts"
);

...
import type { Filters, ListRow, Tab } from "@/backend/queries";
import { severityOf } from "@/backend/definitions";
import { IST } from "@/backend/definitions";
...
const TAT_COLS: Col[] = [
  ...
  { header: "Courier Attempts", width: 15, kind: "int" },
...
function tatValues(r: ListRow, sr: number): unknown[] {
  return [
    ...
    r.payment_type, r.order_total, r.ndr_reason ?? "—", r.attempt_count, r.courier_attempts ?? "—",
    r.warehouse, r.seller_name,
  ];
}

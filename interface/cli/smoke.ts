...
async function main() {
  const { listTools, callTool } = await import("../mcp");
...
main().catch((err) => {
  ...
  // `fetch failed` (undici) carries the real reason — DNS/TLS/ECONNRESET/timeout —
  // only on `.cause`; without this a transient blip is indistinguishable from a
  // dead endpoint or a bad token.
